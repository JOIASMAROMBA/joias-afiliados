...
export default function LoginPage() {
  ...
  const handleLogin = async (e) => {
    ...
    const { data, error: dbError } = await supabase
      ...
      .ilike('coupon_code', coupon.trim())
      .single();
    if (dbError || !data) {
      setError('Cupom não encontrado. Verifique e tente novamente.');
      setLoading(false);
      return;
    }
    localStorage.setItem('affiliate_id', data.id);
    localStorage.setItem('affiliate_name', data.name);
    localStorage.setItem('affiliate_coupon', data.coupon_code);
    router.push('/painel');
  };
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, background: 'linear-gradient(135deg, #0f0520 0%, #1a0a2e 40%, #0d0a1a 100%)' }}>
      <div style={{ width: '100%', maxWidth: 400, animation: 'slideUp 0.5s ease-out' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: 14 }}>Divulgue e ganhe R$30 por peça vendida</p>
        </div>
        <div style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 24, padding: 32 }}>
          <label style={{ display: 'block', marginBottom: 8, color: 'rgba(255,255,255,0.6)', fontSize: 13, fontWeight: 600 }}>Seu cupom de afiliado</label>
          <input type="text" value={coupon} onChange={(e) => setCoupon(e.target.value.toUpperCase())} placeholder="Ex: CAMILA15" style={{ width: '100%', padding: '16px 20px', background: 'rgba(255,255,255,0.08)', border: '2px solid rgba(255,215,0,0.2)', borderRadius: 14, color: '#FFD700', fontSize: 20, fontWeight: 800, textAlign: 'center', letterSpacing: 2, outline: 'none', fontFamily: "'DM Sans', sans-serif" }} />
        ...
        <div style={{ textAlign: 'center', marginTop: 24, color: 'rgba(255,255,255,0.3)', fontSize: 12 }}>
          Ainda não tem cupom?{' '}<span onClick={() => router.push('/cadastro')} style={{ color: '#FFD700', cursor: 'pointer', fontWeight: 600 }}>Quero ser afiliado(a)</span>
        </div>
      </div>
    </div>
  );
