...
export default function PainelPage() {
  ...
  async function openMaterialFolder(folder) {
    ...
    setLoadingMaterials(true);
    try {
      const res = await fetch('/api/materials/files?folder_id=' + encodeURIComponent(folder.id) + '&t=' + Date.now(), { cache: 'no-store' });
      const data = await res.json();
      ...
      {activeTab === 'home' && (
        <div className="painel-home-grid">
          ...
          <div className="saldos-card-spacing" style={{ position: 'relative', background: 'rgba(15,15,15,0.6)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', border: '2px solid #00ff88', borderRadius: 16, padding: '14px 18px 16px', marginBottom: 16, boxShadow: '0 0 30px rgba(0,255,136,0.15)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10, marginBottom: 10, marginTop: fixedMonthly && fixedMonthly.active ? 8 : 0 }}>
              <div>
          ...
          <div className="full-width" style={{ background: 'rgba(15,15,15,0.6)', backdropFilter: 'blur(40px)', WebkitBackdropFilter: 'blur(40px)', border: '1px solid rgba(201,169,97,0.2)', borderRadius: 16, padding: 18, marginBottom: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, marginBottom: 12 }}>
              {weekDays.map(function(d, i) {
                ...
                return (
                  <div key={i} style={{ background: bg, border: border, borderRadius: 10, padding: '8px 4px', textAlign: 'center', opacity: opacity, position: 'relative', minHeight: 70 }}>
                    ...
                    {label === 'obrigatory' && (
                      <>
                        <div style={{ fontSize: 7, color: '#C9A961', fontWeight: 800, marginTop: 11 }}>DIA DE</div>
