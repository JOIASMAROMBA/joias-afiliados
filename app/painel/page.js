          ...
          <div style={{ maxWidth: 400, width: '100%', background: 'linear-gradient(145deg, #1a0a2e 0%, #2d1b4e 100%)', border: '2px solid #FFD700', borderRadius: 24, padding: 28, animation: 'slideUp 0.4s ease-out' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 14 }}>
              {['CPF','Email','Telefone','Aleatoria'].map(function(t) {
                return (
                  <button key={t} onClick={function() { setPixType(t); }} style={{ padding: 10, borderRadius: 10, border: pixType === t ? '2px solid #FFD700' : '1px solid rgba(255,255,255,0.1)', background: pixType === t ? 'rgba(255,215,0,0.1)' : 'rgba(255,255,255,0.03)', color: pixType === t ? '#FFD700' : 'rgba(255,255,255,0.5)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans', sans-serif" }}>{t}</button>
                );
              })}
            </div>
            ...
            <div style={{ textAlign: 'center', marginTop: 10, color: 'rgba(255,255,255,0.4)', fontSize: 11 }}>Processamento em ate 24h uteis</div>
          </div>
        </div>
      )}
