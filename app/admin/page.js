    ...
    <div style={{ minHeight: '100vh', background: '#FAFAFA', color: '#1A1A1A', fontFamily: '-apple-system, BlinkMacSystemFont, Segoe UI, sans-serif', display: 'flex' }}>
      ...
      <main className="admin-content" style={{ flex: 1, minWidth: 0, padding: 24, overflow: 'hidden' }}>
        ...
        {activeTab === 'vendas-manual' && (function() {
          ...
          var lmDt = lastManual && lastManual.created_at ? new Date(lastManual.created_at) : null;
          var lmDtStr = lmDt ? lmDt.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', year: '2-digit' }) + ' ' + lmDt.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' }) : null;
          var lmCupom = lastManual && lastManual.affiliates && lastManual.affiliates.coupon_code ? lastManual.affiliates.coupon_code : null;

          return (<div>
            <div style={{ background: '#0a0a0a', border: '1px solid #1f1f1f', borderRadius: 12, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 16, marginBottom: 14, boxShadow: '0 6px 20px rgba(0,0,0,0.35)', flexWrap: 'wrap' }}>
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 16, fontWeight: 900, color: '#FFD700', lineHeight: 1.1, fontVariantNumeric: 'tabular-nums' }}>{lmDtStr || '—'}</div>
                {lmCupom && (<div style={{ fontSize: 9, color: 'rgba(255,255,255,0.5)', fontFamily: 'monospace', letterSpacing: 1, marginTop: 3 }}>{lmCupom}</div>)}
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  style={{ width: 140, padding: '6px 10px', background: '#000', border: '1px solid rgba(255,215,0,0.35)', borderRadius: 6, color: '#FFD700', fontSize: 13, fontWeight: 700, outline: 'none', fontFamily: 'monospace' }}
                />
              </div>
            </div>
