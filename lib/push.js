...
function ensureConfigured() {
  ...
  var pub = (process.env.VAPID_PUBLIC_KEY || process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY || '').trim();
  var priv = (process.env.VAPID_PRIVATE_KEY || '').trim();
  var subject = (process.env.VAPID_SUBJECT || 'mailto:renanforumn@gmail.com').trim();
...
export async function sendPushToAffiliate(affiliateId, payload) {
  ...
  for (var i = 0; i < subs.length; i++) {
    ...
    try {
      ...
      errors.push({ statusCode: sc, message: err && err.message ? err.message : String(err), body: err && err.body });
  ...
  return { ok: true, sent: sent, removed: deadIds.length, total: subs.length, errors: errors };
}
