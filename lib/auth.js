...
export async function requireSession(request) {
  ...
  const { data: affiliate, error } = await supabaseAdmin
    .from('affiliates')
    .select('id, name, coupon_code, is_admin, blocked, deleted_at, gender')
    .eq('id', session.id)
    .maybeSingle();
  if (error) return { error: 'db_error', status: 500 };
  if (!affiliate || affiliate.blocked || affiliate.deleted_at) return { error: 'invalid_session', status: 401 };
