...
export function buildResetPasswordEmail({ name, coupon, provisionalPassword, gender }) {
  const isMale = gender === 'male';
  const firstName = (name || '').split(' ')[0] || (isMale ? 'Afiliado' : 'Afiliada');
  const inner = `
...
export function buildWithdrawalCodeEmail({ name, coupon, code, amount, pixType, pixKey, gender }) {
  const isMale = gender === 'male';
  const firstName = (name || '').split(' ')[0] || (isMale ? 'Afiliado' : 'Afiliada');
...
export function buildApprovalEmail({ name, coupon, gender }) {
  const isMale = gender === 'male';
  const firstName = (name || '').split(' ')[0] || (isMale ? 'Afiliado' : 'Afiliada');
  const approvedWord = isMale ? 'aprovado' : 'aprovada';
  const inner = `
    <div style="text-align:center;margin-bottom:18px;"><div style="font-size:44px;">🎉</div><h1 style="color:#C9A961;font-size:22px;margin:6px 0;">Cadastro ${approvedWord}!</h1></div>
    <p style="font-size:15px;line-height:1.6;color:#fff;margin:0 0 10px;">Oi ${firstName},</p>
    <p style="font-size:14px;line-height:1.6;color:rgba(255,255,255,0.85);margin:0 0 14px;">
      Parabéns! Seu cadastro no Programa de Afiliados Joias Maromba foi <strong style="color:#C9A961;">${approvedWord}</strong>.
...
export function buildRejectionEmail({ name, reason }) {
  ...
  const inner = `
    ...
    <p style="font-size:15px;line-height:1.6;color:#fff;margin:0 0 10px;">Oi ${firstName},</p>
    <p style="font-size:14px;line-height:1.6;color:rgba(255,255,255,0.85);margin:0 0 14px;">
      Infelizmente seu cadastro no Programa de Afiliados Joias Maromba não foi aprovado neste momento.
    </p>
...
export function buildWithdrawalCreatedEmail({ name, amount, pixType, pixKey, ip, gender }) {
  const isMale = gender === 'male';
  const firstName = (name || '').split(' ')[0] || (isMale ? 'afiliado' : 'afiliada');
  const valueStr = 'R$ ' + Number(amount || 0).toFixed(2).replace('.', ',');
  const inner = `
